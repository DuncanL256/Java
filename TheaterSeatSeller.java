...
public class TheaterSeatSeller {
    ...
    public TheaterSeatSeller() {
        ...
        for(int i = 0; i < seats[0].length-1 ; i++) {
            for(int j = 0; j < seats.length; j++) {
                //int checkNum = seats[i][j];
                if(seats[i][j] == 10){
    ...
    public boolean getByPrice(int price) {
        ...
        if(price==10 && pricesAvailable[0] > 0){
            pricesAvailable[0] = pricesAvailable[0] - 1;
            retVal = true;
        } else if(price==20 && pricesAvailable[1] > 0){
            pricesAvailable[1] = pricesAvailable[1] - 1;
            retVal = true;
        } else if(price==30 && pricesAvailable[2] > 0){
            pricesAvailable[2] = pricesAvailable[2] - 1;
            retVal = true;
        } else if(price==40 && pricesAvailable[3] > 0){
            pricesAvailable[3] = pricesAvailable[3] - 1;
            retVal = true;
        } else if(price==50 && pricesAvailable[4] > 0){
            pricesAvailable[4] = pricesAvailable[4] - 1;
            retVal = true;
        } else{
    ...
    public void start() { //initiates the simulation
        ...
        while(running) {//main execution loop
            ...
            System.out.println();
            if(userInput.equals("10")||userInput.equals("20")||
            userInput.equals("30")
            ||userInput.equals("40")||
            userInput.equals("50")) {
